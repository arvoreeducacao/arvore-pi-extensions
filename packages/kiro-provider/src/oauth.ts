...
export async function loginKiro(
	...
	if (!process.env.VITEST) {
		try {
			const { resolveApiRegion, updateKiroModelsCache } = await import(
				"./models.js"
			);
			...
			updateKiroModelsCache(
				creds.access,
				region,
				(creds as KiroCredentials).profileArn,
			).catch(() => {});
		} catch {
			// Ignore cache errors
		}
...
async function loginKiroInternal(
	...
	if (
		...
		).onProgress?.("Using existing Kiro IDE credentials");
		return ideCreds;
	}
	...
	if (expiredIdeCreds) {
		try {
			(
				callbacks as unknown as { onProgress?: (msg: string) => void }
			).onProgress?.("Refreshing Kiro IDE credentials...");
	...
	if (expiredCreds) {
		try {
			(
				callbacks as unknown as { onProgress?: (msg: string) => void }
			).onProgress?.("Refreshing expired kiro-cli credentials...");
 ...
 */
export async function loginKiroBuilderID(
	callbacks: OAuthLoginCallbacks,
): Promise<OAuthCredentials> {
...
async function refreshKiroTokenInternal(
	credentials: OAuthCredentials,
): Promise<OAuthCredentials> {
	const {
		getKiroCliCredentials,
...
async function refreshKiroTokenDirect(
	credentials: OAuthCredentials,
): Promise<OAuthCredentials> {
	...
	if (authMethod === "desktop") {
		...
		return {
			...
			profileArn:
				data.profileArn || (credentials as KiroCredentials).profileArn,
		};
	}
