	...
	try {
		...
		if (!arn) {
			debugLog("profileArn.empty", {
				message:
					"ListAvailableProfiles returned no profile ARN; this is expected for some social-login tokens.",
			});
			return undefined;
		}
...
export function streamKiro(
	...
	(async () => {
		...
		try {
			...
			const cliCreds =
				getKiroCliCredentials() ?? getKiroCliCredentialsAllowExpired();
			const cliProfileArn =
				cliCreds?.access === accessToken ? cliCreds.profileArn : undefined;
			let profileArn =
				optionProfileArn ||
				cliProfileArn ||
			...
			const { isCacheStale, updateKiroModelsCache } = await import(
				"./models.js"
			);
			...
			while (retryCount <= maxRetries) {
						...
						if (m.role === "toolResult") {
							...
							if (Array.isArray(trm.content))
								for (const c of trm.content)
									if (c.type === "image")
										toolResultImages.push(c as ImageContent);
						}
					}
					...
					for (const m of currentMessages)
						if (m.role === "toolResult") {
							...
							if (Array.isArray(trm.content))
								for (const c of trm.content)
									if (c.type === "image")
										toolResultImages2.push(c as ImageContent);
						}
				...
				if (firstMsg?.role === "user") {
					...
					if (imgs.length > 0)
						currentImages = convertImagesToKiro(imgs as ImageContent[]);
				}
				...
				if (capacityRetryCount > 0 && response.ok) {
					logCapacityEvent(
						`INSUFFICIENT_MODEL_CAPACITY — succeeded after ${capacityRetryCount} retries`,
					);
				}
				...
				const hasText =
					textBlockIndex !== null &&
					(output.content[textBlockIndex] as TextContent).text.length > 0;
				...
				debugLog("response.done", {
					...
					textLen:
						textBlockIndex !== null
							? (output.content[textBlockIndex] as TextContent).text.length
							: 0,
					usage: output.usage,
