...
export async function showLoginUI(): Promise<LoginChoice> {
	...
	return ctx.ui.custom<LoginChoice>((tui, theme, _kb, done) => {
		...
		const borderBottom = new DynamicBorder((s: string) =>
			theme.fg("accent", s),
		);
		...
		// Phase 2: URL Input
		const urlLabel = new Text(
			"Start URL (e.g. https://mycompany.awsapps.com/start)",
			1,
			0,
		);
		...
		function switchToUrlInput() {
			...
			container.addChild(
				new Text(theme.fg("accent", theme.bold("IAM Identity Center")), 1, 0),
			);
