...
function getProgress(
	...
	return (callbacks as unknown as { onProgress?: (msg: string) => void })
		.onProgress;
}
...
export async function interactiveLogin(
	...
	if (!input.startsWith("http"))
		throw new Error(
			`Invalid input "${input}". Paste your start URL or leave blank for Builder ID.`,
		);
			...
			case undefined:
				if (tokData.accessToken && tokData.refreshToken) {
					return {
						...
						expires:
							Date.now() + (tokData.expiresIn || 3600) * 1000 - 5 * 60 * 1000,
						clientId,
						clientSecret,
						region,
...
export async function loginViaKiroCli(
	...
	try {
		...
		throw new Error(
			`kiro-cli login failed: ${msg}. Ensure kiro-cli is installed and in PATH.`,
		);
	}
	...
	if (!creds)
		throw new Error(
			"kiro-cli login completed but no credentials found in its database",
		);

	getProgress(callbacks)?.(
		creds.authMethod === "desktop"
