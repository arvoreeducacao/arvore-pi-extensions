...
function mapBucket(
	bucket: KiroUsageBreakdown,
	index: number,
): KiroProviderUsageBucket {
	...
	const freeTrialUsed =
		bucket.freeTrialInfo?.currentUsageWithPrecision ??
		bucket.freeTrialInfo?.currentUsage;
	const freeTrialLimit =
		bucket.freeTrialInfo?.usageLimitWithPrecision ??
		bucket.freeTrialInfo?.usageLimit;

	return {
		id: bucketId(bucket, index),
		label:
			bucket.displayName ||
			bucket.displayNamePlural ||
			bucket.resourceType ||
			"Usage",
		...
		unit: bucket.unit,
		overagesDisplay:
			overages && overages > 0 ? formatCount(overages) : undefined,
		...
		bonus:
			...
			bucket.freeTrialInfo?.freeTrialExpiry !== undefined
				? {
						label: "Bonus credits",
...
async function postOperation<TResponse>(
	...
	if (!response.ok) {
		...
		throw new Error(
			`${target} failed: ${response.status} ${response.statusText}${text ? ` ${text}` : ""}`,
		);
	}
...
async function fetchRawUsage(
	credentials: OAuthCredentials,
): Promise<KiroGetUsageLimitsResponse> {
	const errors: string[] = [];

	const direct = await tryUsageBodies(
		credentials,
		buildUsageBodies(undefined),
		errors,
	);
	...
	if (profileArn) {
		const profiled = await tryUsageBodies(
			credentials,
			buildUsageBodies(profileArn),
			errors,
		);
