...
function encodeBody(body: string): Uint8Array {
	return concatMessages(
		...parseJsonObjects(body).map((o) => encodeEventMessage(o)),
	);
}
...
function mockFetchChunked(chunks: string[]) {
	...
	return vi.fn().mockResolvedValueOnce({
		ok: true,
		body: {
			getReader: () => ({ read: readMock, releaseLock: () => {} }),
			cancel: async () => {},
		},
	});
}
...
describe("Feature 9: Streaming Integration", () => {
	...
	it("makes POST to correct endpoint with auth header", async () => {
		const mockFetch = mockFetchOk(
			'{"content":"Hi"}{"contextUsagePercentage":10}',
		);
		...
		expect(
			msg?.content.some((b) => b.type === "text" && b.text.includes("Hi")),
		).toBe(true);
	...
	it("resolves profileArn via ListAvailableProfiles and includes it in request body", async () => {
					...
					getReader: () => ({
						read: vi
							...
							.mockResolvedValueOnce({
								...
								value: encodeBody(
									'{"content":"Hi"}{"contextUsagePercentage":5}',
								),
							})
							.mockResolvedValueOnce({ done: true, value: undefined }),
						releaseLock: () => {},
		...
		expect(mockFetch.mock.calls[0][1].headers["X-Amz-Target"]).toBe(
	...
	it("sets stopReason to toolUse when tool calls are present", async () => {
		const toolPayload =
			'{"name":"bash","toolUseId":"tc1","input":"{\\"cmd\\":\\"ls\\"}","stop":true}';
		const mockFetch = mockFetchOk(
			`${toolPayload}{"contextUsagePercentage":20}`,
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("emits complete text_start -> text_delta -> text_end sequence", async () => {
		...
		const done = events.find((e) => e.type === "done");
		const msg = done?.type === "done" ? done.message : undefined;
		expect(msg?.content[0].type === "text" && msg.content[0].text).toBe(
			"Hello world",
		);
	...
	it("does not withhold the tail of plain text in reasoning mode", async () => {
		...
		expect(firstTextDelta?.type === "text_delta" && firstTextDelta.delta).toBe(
			"Hello world",
		);
	...
	it("emits toolcall_start -> toolcall_delta -> toolcall_end with parsed arguments", async () => {
		const toolPayload =
			'{"name":"bash","toolUseId":"tc1","input":"{\\"cmd\\":\\"ls\\"}","stop":true}';
		const mockFetch = mockFetchOk(
			`{"content":"Let me run that."}${toolPayload}{"contextUsagePercentage":10}`,
		);
		vi.stubGlobal("fetch", mockFetch);
		...
		expect(
			tcEnd?.type === "toolcall_end" &&
				(tcEnd.toolCall.arguments as Record<string, unknown>).cmd,
		).toBe("ls");
	...
	it("emits tool calls as they arrive instead of waiting for stream end", async () => {
		...
		expect(
			tcEnd?.type === "toolcall_end" &&
				(tcEnd.toolCall.arguments as Record<string, unknown>).path,
		).toBe("file.txt");
	...
	it("handles multiple tool calls in a single response", async () => {
		const tool1 =
			'{"name":"bash","toolUseId":"tc1","input":"{\\"cmd\\":\\"ls\\"}","stop":true}';
		const tool2 =
			'{"name":"read","toolUseId":"tc2","input":"{\\"path\\":\\"f.txt\\"}","stop":true}';
		const mockFetch = mockFetchOk(
			`${tool1}${tool2}{"contextUsagePercentage":10}`,
		);
		vi.stubGlobal("fetch", mockFetch);
		...
		expect(tcEnds[0].type === "toolcall_end" && tcEnds[0].toolCall.name).toBe(
			"bash",
		);
		expect(tcEnds[1].type === "toolcall_end" && tcEnds[1].toolCall.name).toBe(
			"read",
		);
	...
	it("totalTokens equals input + output", async () => {
		const mockFetch = mockFetchOk(
			'{"content":"Hello there, this is a response."}{"contextUsagePercentage":8}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("emits aborted when signal fires mid-stream", async () => {
		...
		expect(
			error?.type === "error" && error.error.content.length,
	...
	it("handles aborted assistant message in context followed by new request", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"Sure!"}{"contextUsagePercentage":5}',
		);
		vi.stubGlobal("fetch", mockFetch);
		...
		expect(
			done?.type === "done" && done.message.content.length,
		).toBeGreaterThan(0);
	...
	it("handles empty string user message", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"Hi"}{"contextUsagePercentage":2}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("handles empty content array user message", async () => {
		const context: Context = {
			...
			messages: [
				{
					role: "user" as const,
					content: [] as (TextContent | ImageContent)[],
					timestamp: ts,
				},
			],
		};
	...
	it("handles empty assistant message in conversation context", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"Here I am"}{"contextUsagePercentage":5}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("strips images from history entries so they don't bloat the request", async () => {
		...
		const context: Context = {
			...
			messages: [
				{
					role: "user",
					content: [{ type: "text", text: "Look at this" }, imageContent],
					timestamp: ts,
				},
				{
					role: "assistant",
		...
		const mockFetch = mockFetchOk(
			'{"content":"It was orange."}{"contextUsagePercentage":5}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("handles multi-turn with images without exceeding size limits", async () => {
		...
		const context: Context = {
			...
			messages: [
				{
					role: "user",
					content: [{ type: "text", text: "Image 1" }, largeImage],
					timestamp: ts,
				},
				{
					role: "assistant",
				...
				{
					role: "user",
					content: [{ type: "text", text: "Image 2" }, largeImage],
					timestamp: ts,
				},
				{
					role: "assistant",
		...
		const mockFetch = mockFetchOk(
			'{"content":"Both were photos."}{"contextUsagePercentage":5}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("handles assistant with tool calls followed by user message (no tool results)", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"4"}{"contextUsagePercentage":5}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("handles full multi-turn: user -> assistant(toolCall) -> toolResult -> assistant(text)", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"The answer is 4."}{"contextUsagePercentage":8}',
		);
		vi.stubGlobal("fetch", mockFetch);
		...
		expect(currentMsg.userInputMessageContext.toolResults[0].toolUseId).toBe(
			"tc1",
		);
	...
	it("synthesizes placeholder tool specs when context.tools is [] but history references tools", async () => {
		...
		const context: Context = {
			...
			messages: [
				...
				{
					role: "user",
					content: "Now please advise on the situation above.",
					timestamp: ts,
				},
			],
			tools: [],
		...
		const mockFetch = mockFetchOk(
			'{"content":"Sure."}{"contextUsagePercentage":3}',
		);
		vi.stubGlobal("fetch", mockFetch);
		...
		const tools = body.conversationState.currentMessage.userInputMessage
			.userInputMessageContext?.tools as
	...
	it("omits userInputMessageContext.tools when context.tools is [] and history has no tool uses", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"Hi"}{"contextUsagePercentage":1}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("retries INSUFFICIENT_MODEL_CAPACITY with backoff then throws after max retries", async () => {
		...
		try {
			...
			expect(error?.type === "error" && error.error.errorMessage).not.toContain(
				"429",
			);
		} finally {
	...
	it("sanitizes unicode surrogates in user message content", async () => {
		const mockFetch = mockFetchOk(
			'{"content":"Got it"}{"contextUsagePercentage":3}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("handles tool call events where toolUseId comes before name", async () => {
		...
		const toolPayload =
			'{"toolUseId":"tc1","name":"write","input":"{\\"path\\":\\"f.txt\\"}","stop":true}';
		const mockFetch = mockFetchOk(
			`${toolPayload}{"contextUsagePercentage":10}`,
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("handles toolUse with input:{} placeholder followed by toolUseInput events", async () => {
		...
		expect(
			tcEnd?.type === "toolcall_end" &&
				(tcEnd.toolCall.arguments as Record<string, unknown>).content,
		).toBe("hello");
	...
	it("logs warning when tool input JSON.parse fails", async () => {
		const toolPayload =
			'{"name":"bash","toolUseId":"tc1","input":"not-valid-json","stop":true}';
		const mockFetch = mockFetchOk(
			`${toolPayload}{"contextUsagePercentage":10}`,
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("handles tool call with empty input string", async () => {
		const toolPayload =
			'{"name":"bash","toolUseId":"tc1","input":"","stop":true}';
		const mockFetch = mockFetchOk(
			`${toolPayload}{"contextUsagePercentage":10}`,
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("works without system prompt", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"Hello"}{"contextUsagePercentage":2}',
		);
		vi.stubGlobal("fetch", mockFetch);

		const stream = streamKiro(makeModel({ reasoning: false }), context, {
			apiKey: "tok",
		});
		const events = await collect(stream);
		const done = events.find((e) => e.type === "done");
	...
	it("does not produce unhandled rejection when reader.cancel() rejects", async () => {
		...
		const existingListeners = process.rawListeners("unhandledRejection") as ((
			...args: unknown[]
		) => void)[];
		...
		expect(
			events.find((e) => e.type === "error" || e.type === "done"),
		).toBeDefined();
	...
	it("refreshes token from kiro-cli on 403 before retrying", async () => {
		...
		const getCredsSpy = vi
			.spyOn(kiroCliModule, "getKiroCliCredentials")
			.mockReturnValue({
	...
	it("passes through contextPercent even without usage event", async () => {
		const mockFetch = mockFetchOk(
			'{"content":"Hi"}{"contextUsagePercentage":42}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("sets stopReason to length when stream ends without contextUsage event", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"partial response that got cut off"}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("prepends truncation notice when previous response was truncated", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"...the rest of the story."}{"contextUsagePercentage":5}',
		);
		vi.stubGlobal("fetch", mockFetch);
	...
	it("extracts bracket tool calls from content as fallback", async () => {
		...
		expect(toolCalls?.[0].type === "toolCall" && toolCalls?.[0].name).toBe(
			"bash",
		);

		// Text content should have bracket pattern stripped
		const textBlock = msg?.content.find((b) => b.type === "text");
		expect(textBlock?.type === "text" && textBlock.text).not.toContain(
			"[Called",
		);
	...
	it("treats tool calls with empty input as valid zero-arg calls", async () => {
		...
		expect(
				...
				done.message.content.filter((b) => b.type === "toolCall"),
		).toHaveLength(1);
	...
	it("retries on completely empty response (no text, no tool calls)", async () => {
					...
					getReader: () => ({
						read: vi
							...
							.mockResolvedValueOnce({
								...
								value: encodeBody(emptyResponse),
							})
							.mockResolvedValueOnce({ done: true, value: undefined }),
						releaseLock: () => {},
					}),
		...
		expect(
					...
					(b) => b.type === "text" && (b as TextContent).text === "recovered",
				),
		).toBe(true);
	...
	it("retries when model responds with just 'Continue' (echo loop detection)", async () => {
		...
		const goodResponse =
			'{"content":"Here is the actual work."}{"contextUsagePercentage":10}';
		...
		expect(
					...
					(b) =>
						b.type === "text" &&
						(b as TextContent).text === "Here is the actual work.",
				),
		).toBe(true);
	...
	it("detects echo loop for '.', 'continue', 'CONTINUE', ' Continue '", async () => {
		for (const echoText of [
			...
			const goodResponse =
				'{"content":"recovered"}{"contextUsagePercentage":10}';
	...
	it("strips echo text after max retries on persistent 'Continue' responses", async () => {
		...
		const textBlocks =
				...
				? done.message.content.filter((b) => b.type === "text")
				: [];
	...
	it("does NOT treat longer text containing 'continue' as echo loop", async () => {
		const response =
			'{"content":"Let me continue working on this task."}{"contextUsagePercentage":10}';
		...
		expect(
					...
					(b) =>
						b.type === "text" &&
						(b as TextContent).text === "Let me continue working on this task.",
				),
		).toBe(true);
	...
	it("history uses merging instead of synthetic padding — no echoable content", async () => {
		...
		const mockFetch = mockFetchOk(
			'{"content":"Done."}{"contextUsagePercentage":5}',
		);
		vi.stubGlobal("fetch", mockFetch);
		...
		const badPadding = history.filter(
			(h: KiroHistoryEntry) =>
				(h.assistantResponseMessage &&
					/^(Continue|\.)$/i.test(h.assistantResponseMessage.content)) ||
				(h.userInputMessage &&
					/^(Continue|\.)$/i.test(h.userInputMessage.content)),
		);
