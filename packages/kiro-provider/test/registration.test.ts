...
const mockPi = () => {
	...
	return {
		pi: { registerProvider, on: vi.fn() } as unknown as ExtensionAPI,
		registerProvider,
	};
};
...
describe("Feature 1: Extension Registration", () => {
	...
	it.each([
		...
		expect(modified[0].baseUrl).toBe(
			`https://q.${expectedApiRegion}.amazonaws.com/generateAssistantResponse`,
		);
	});
