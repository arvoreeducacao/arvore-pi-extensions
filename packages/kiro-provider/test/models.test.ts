...
describe("Feature 2: Model Definitions", () => {
	describe("resolveKiroModel", () => {
		...
		it("throws on unknown model ID", () => {
			expect(() => resolveKiroModel("nonexistent")).toThrow(
				"Unknown Kiro model ID",
			);
		});
	});
	...
	describe("filterModelsByRegion", () => {
		it("us-east-1 returns all models", () => {
			expect(filterModelsByRegion(kiroModels, "us-east-1")).toHaveLength(
				kiroModels.length,
			);
		});

		it("eu-central-1 includes Claude + documented OSS, excludes DeepSeek and undocumented models", () => {
			const ids = filterModelsByRegion(kiroModels, "eu-central-1").map(
				(m) => m.id,
			);
	...
	describe("model catalog", () => {
		...
		it("claude-haiku-4-5 has reasoning=false", () => {
			expect(
				kiroModels.find((m) => m.id === "claude-haiku-4-5")?.reasoning,
			).toBe(false);
		});
		...
		it("minimax has reasoning=false", () => {
			expect(kiroModels.find((m) => m.id === "minimax-m2-1")?.reasoning).toBe(
				false,
			);
		});
		...
		it("all models have zero cost", () => {
			expect(
				kiroModels.every((m) => m.cost.input === 0 && m.cost.output === 0),
			).toBe(true);
		});
		...
		it("opus models have expected max tokens", () => {
			...
			expect(
				opusModels.every(
					(m) => m.maxTokens === 32768 || m.maxTokens === 128000,
				),
			).toBe(true);
		});
