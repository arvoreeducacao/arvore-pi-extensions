...
describe("Feature 3: OAuth — Token Refresh", () => {
	...
	describe("refreshKiroToken", () => {
		...
		it("throws on failed refresh", async () => {
			...
			await expect(
				refreshKiroToken({ refresh: "rt|c|s|idc", access: "x", expires: 0 }),
			).rejects.toThrow();
			vi.unstubAllGlobals();
		});
		...
		it("refreshes desktop tokens via Kiro auth service", async () => {
			const mockFetch = vi.fn().mockResolvedValueOnce({
				ok: true,
				json: () =>
					Promise.resolve({ accessToken: "desk_at", expiresIn: 3600 }),
			});
			vi.stubGlobal("fetch", mockFetch);
		...
		it("uses expired kiro-cli creds as fallback when direct refresh fails", async () => {
			const { getKiroCliCredentialsAllowExpired } = await import(
				"../src/kiro-cli.js"
			);
