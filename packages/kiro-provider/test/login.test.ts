...
function makeCallbacks(
	...responses: string[]
): OAuthLoginCallbacks & { onAuth: ReturnType<typeof vi.fn> } {
...
function mockBuilderIdFetch() {
	return vi
		...
		.mockResolvedValueOnce({
			ok: true,
			json: () => Promise.resolve({ clientId: "cid", clientSecret: "csec" }),
		})
		.mockResolvedValueOnce({
			ok: true,
			json: () =>
				Promise.resolve({
					...
					verificationUriComplete:
						"https://device.sso.us-east-1.amazonaws.com?code=ABCD",
					userCode: "ABCD-1234",
					deviceCode: "dc",
					interval: 1,
					expiresIn: 10,
...
function mockIdcAutoDetectFetch() {
	return vi
		...
		.mockResolvedValueOnce({
			ok: true,
			json: () => Promise.resolve({ clientId: "c1", clientSecret: "s1" }),
		}) // us-east-1 register
		.mockResolvedValueOnce({ ok: false, status: 400 }) // us-east-1 device_auth → wrong region
		.mockResolvedValueOnce({
			ok: true,
			json: () => Promise.resolve({ clientId: "c2", clientSecret: "s2" }),
		}) // eu-west-1 register
		.mockResolvedValueOnce({
			ok: true,
			json: () =>
...
describe("Feature 10: Interactive Login", () => {
	describe("method selection (prompt 1)", () => {
		it("blank/1 → Builder ID", async () => {
			...
			expect(JSON.parse(mockFetch.mock.calls[1][1].body).startUrl).toBe(
		...
		it("TUI: google → kiro-cli Google", async () => {
			...
			const { getKiroCliSocialToken, getKiroCliCredentials } = await import(
				"../src/kiro-cli.js"
			);
			...
			expect(
				((await interactiveLogin(makeCallbacks(""))) as KiroCredentials)
					.authMethod,
			).toBe("desktop");
		});
		...
		it("fallback: invalid non-URL input → throws", async () => {
			...
			await expect(interactiveLogin(makeCallbacks("notaurl"))).rejects.toThrow(
				"Invalid input",
			);
			vi.unstubAllGlobals();
		});
	...
	describe("fallback — onPrompt path (no TUI ctx)", () => {
		it("URL → auto-detects region", async () => {
			...
			const creds = await interactiveLogin(
				makeCallbacks("https://mycompany.awsapps.com/start"),
			);
			expect((creds as KiroCredentials).region).toBe("eu-west-1");
			vi.unstubAllGlobals();
		});
				...
				vi.fn().mockImplementation(() => {
					...
					if (call % 2 === 1)
						return Promise.resolve({
							ok: true,
							json: () => Promise.resolve({ clientId: "c", clientSecret: "s" }),
						});
					return Promise.resolve({ ok: false, status: 400 });
				}),
			);
			await expect(
				interactiveLogin(makeCallbacks("https://unknown.awsapps.com/start")),
