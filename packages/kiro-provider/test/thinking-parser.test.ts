...
describe("Feature 7: Thinking Tag Parser", () => {
	...
	it("flushes plain text immediately without waiting for finalize", () => {
		...
		expect(output.content[0]?.type === "text" && output.content[0].text).toBe(
			"Hello world",
		);
	});
	...
	it("retains only a trailing possible opening-tag prefix between chunks", () => {
		...
		expect(
			output.content[0]?.type === "thinking" && output.content[0].thinking,
		).toBe("deep thought");
		expect(output.content[1]?.type === "text" && output.content[1].text).toBe(
			"Hello ",
		);
	});
	...
	it("reorders thinking before text when text arrives first", async () => {
		...
		expect((output.content[0] as { thinking: string }).thinking).toBe(
			"reasoning",
		);
	...
	it("handles text-before-thinking across multiple chunks", async () => {
		...
		expect((output.content[1] as { text: string }).text).toBe(
