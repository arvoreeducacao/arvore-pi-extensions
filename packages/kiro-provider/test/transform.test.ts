...
describe("Feature 5: Message Transformation", () => {
	...
	describe("normalizeMessages", () => {
		it("filters errored assistant messages", () => {
			const msgs: Message[] = [
				user("hi"),
				assistant("oops", { stopReason: "error" }),
				user("retry"),
			];
		...
		it("filters aborted assistant messages", () => {
			expect(
				normalizeMessages([
					user("hi"),
					assistant("x", { stopReason: "aborted" }),
				]),
			).toHaveLength(1);
		});
	...
	describe("convertToolsToKiro", () => {
		it("converts pi tools to Kiro specs", () => {
			...
			expect(r[0].toolSpecification.inputSchema.json).toEqual(
				tools[0].parameters,
			);
		});
	});
	...
	describe("buildHistory", () => {
		...
		it("prepends system prompt to first user message", () => {
			...
			const { history, systemPrepended } = buildHistory(
				msgs,
				"M",
				"Be helpful",
			);
		...
		it("truncates tool results exceeding limit", () => {
			...
			const text =
				entry?.userInputMessage?.userInputMessageContext?.toolResults?.[0]
					.content[0].text ?? "";
