	...
	for (const line of lines) {
		...
		try {
			...
			if (obj.contextUsagePercentage !== undefined) {
				events.push({
					type: "contextUsage",
					data: { contextUsagePercentage: obj.contextUsagePercentage },
				});
			} else if (obj.assistantResponseEvent?.content !== undefined) {
				events.push({
					type: "content",
					data: obj.assistantResponseEvent.content,
				});
			}
		} catch {}
	}
...
async function sendRequest(
	...
	return {
		...
		responseSnippet: responseSnippet.slice(0, 100),
	};
}
...
async function main() {
	...
	console.log(
		`Payload size: ${content.length} chars (~${approxTokens} tokens)\n`,
	);

	const models = [
		{
			id: "claude-sonnet-4.5",
			label: "Sonnet 4.5 (200k)",
			expectedWindow: 200_000,
		},
		{
			id: "claude-sonnet-4.5-1m",
			label: "Sonnet 4.5 1M",
			expectedWindow: 1_000_000,
		},
	];
	...
	for (const model of models) {
		...
		try {
			...
			if (pct !== null) {
				...
				console.log(
					`  Implied context window: ~${(impliedWindow / 1000).toFixed(0)}k tokens`,
				);
				console.log(
					`  Expected context window: ${(model.expectedWindow / 1000).toFixed(0)}k tokens`,
				);

				const ratio = impliedWindow / model.expectedWindow;
				if (ratio < 0.5 || ratio > 2.0) {
					console.log(
						`  ⚠️  MISMATCH — implied window is ${ratio.toFixed(1)}x expected`,
					);
				} else {
